...
$sqlBorrowed = "SELECT COUNT(*) AS count FROM borrow WHERE status = 0 AND DATE(date_borrow) BETWEEN '$firstDayOfMonth' AND '$lastDayOfMonth'";
$resultBorrowed = mysqli_query($con, $sqlBorrowed);
$rowBorrowed = mysqli_fetch_assoc($resultBorrowed);
$borrowed = $rowBorrowed['count'];

$sqlReturned = "SELECT COUNT(*) AS count FROM borrow WHERE status = 1 AND DATE(date_return) BETWEEN '$firstDayOfMonth' AND '$lastDayOfMonth'";
